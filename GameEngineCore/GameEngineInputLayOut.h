...
public:
	...
	void AddInputLayoutDesc(LPCSTR _SemanticName, DXGI_FORMAT _Format, UINT _SemanticIndex = 0, UINT _AlignedByteOffset = -1, D3D11_INPUT_CLASSIFICATION _InputSlotClass = D3D11_INPUT_PER_VERTEX_DATA, UINT _InputSlot = 0, UINT _InstanceDataStepRate = 0);
};
...
public:
	...
	void ResCreate(std::shared_ptr<GameEngineVertexBuffer> _Buffer, std::shared_ptr<GameEngineVertexShader> _Shader);
	void Setting();
private:
